...
namespace bm176
{
    ...
    void BMToggleSwitch::paint(juce::Graphics& g)
    {
        ...
        shadowPath.addEllipse(juce::Rectangle<float>(cx - nutFlats * 0.6f, cy - nutFlats * 0.4f,
        ...
        juce::ColourGradient nutGrad(
            nutTop, cx - nutFlats * 0.25f, cy - nutH * 0.5f,
            nutBottom, cx + nutFlats * 0.25f, cy + nutH * 0.5f, false);
        juce::Path nut;
        const float nutR = nutFlats * 0.65f;
        nut.addEllipse(juce::Rectangle<float>(cx - nutR, cy - nutH * 0.35f, nutR * 2.0f, nutH * 0.8f));
        g.setGradientFill(nutGrad);
        g.fillPath(nut);
        g.setColour(juce::Colours::black);
        g.drawEllipse(cx - nutR, cy - nutH * 0.35f, nutR * 2.0f, nutH * 0.8f, 1.0f);

        const float collarW = 18.0f;
        const float collarH = 8.0f;
        const float collarY = -9.0f;
        juce::ColourGradient collarGrad(
            chromeTop, cx - collarW * 0.3f, cy + collarY - collarH * 0.5f,
            chromeMid, cx + collarW * 0.3f, cy + collarY + collarH * 0.5f, false);
        juce::Path collar;
        collar.addEllipse(juce::Rectangle<float>(cx - collarW * 0.5f, cy + collarY - collarH * 0.5f,
            collarW, collarH));
        g.setGradientFill(collarGrad);
        g.fillPath(collar);

        const float pivotX = cx;
        const float pivotY = cy - 12.0f;
        const float batLen = state ? 34.0f : 28.0f;
